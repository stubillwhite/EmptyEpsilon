...
: GuiElement(owner, id)
{
    ...
    signal_label = new GuiLabel(box, id + "_LABEL", tr("Electric signature"), 30);
    ...
    locked_label = new GuiLabel(signal_quality, id + "_LOCK_LABEL", tr("ScanningDialog","LOCKED"), 50);
    ...
    cancel_button = new GuiButton(box, id + "_CANCEL", tr("ScanningDialog","Cancel"), []() {
        if (my_spaceship)
