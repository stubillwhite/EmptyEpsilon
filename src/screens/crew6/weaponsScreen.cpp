...
: GuiOverlay(owner, "WEAPONS_SCREEN", colorConfig.background)
{
    ...
    if (gameGlobalInfo->all_can_be_targeted)
    {
        ...
        LOG(INFO) << "-- NatDebug - WeaponsScreen - weapons_specific_station: " << PreferencesManager::get("weapons_specific_station", "0").toInt() << ", getBeamsButtonStation(): " << my_spaceship->getBeamsButtonStation();
