...
item:setLongDescription([[A nuclear missile is similar to a homing missile in that it can seek a target, but it moves and turns more slowly and explodes a greatly increased payload. Its nuclear explosion spans 1U of space and can take out multiple ships in a single shot. Most factions impose some sort of chain of command embargo on the use of such weapons. Many ships will carry them in case of emergency, but their use usually requires permission from a higher authority, and controls may be locked out unless that permission is given.   
...
item:addKeyValue('Damage at center', '160')
item:addKeyValue('Damage at edge', '30')
item:setLongDescription([[Mines are often placed in defensive perimeters around stations. They are a useful defensive weapon, but can also be a liability to friendly ships. 
...
item:addKeyValue('Damage at center', '160')
item:addKeyValue('Damage at edge', '30')
item:setLongDescription([[The electromagnetic pulse missile (EMP) reproduces the disruptive effects of a nuclear explosion, but without the destructive properties. This causes it to only affect shields within its blast radius, leaving ships hulls intact. The EMP missile is also smaller and easier to store than heavy nukes. Many captains prefer EMPs over nukes for these reasons, and use them to knock out targets' shields before closing to disable them with focused beam fire.]])
...
Each shot from an HVLI fires a burst of 5 projectiles, which increases the chance to hit but requires precision aiming to be effective. These weapons essentially turn missile tubes into a crude form of railgun. They are particularly favoured by the Shanghai Pact, who have perfected the technology and whose crews train extensively in their use.]])
