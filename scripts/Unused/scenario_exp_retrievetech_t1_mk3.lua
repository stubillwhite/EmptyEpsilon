...
-- Description: Person in shuttle variant y'know'. Nat's edit
-- Type: Basic
-- Variation[Takanami]: This scenario will run on Takanami
-- Variation[Havock]: This scenario will run on Havock

-- NB Changes (line references original version of the script):
-- Number of peronnel on Warspite 
-- 1027 >   addGMFunction(GMDisappearDockednew, triggerGMDisappearDockednew)
-- set scarab to indestructable (too much of this scenario relies of it being indestructable soo....)
-- changed zone colour to black 
-- changed template for the station to "Asteroid Mining Rig" (Go to shipTemplates_BSStations.lua and un-comment the lines including this template to avoid the game crashing)

--NICE TO HAVE--

--template that's not called EFS mining rig' -- DONE
...
--zone transparency. cannot figure out -- sort of done? Making it black makes it less intrusive. 
--energy readouts don't update, would need a delta update 
--update ice planet stuff because it's having a visual glitch 
--buggy docking button -- DONE 
...
function deadship()
 ...
 -- PUTTING ENERGY READOUTS IN DELTA 

function getEnemyEnergy()
    mf2Energy = minifight2:getEnergy()
    mf3Energy = minifight3:getEnergy()
    hh1Energy = hacker1:getEnergy()
    h2Energy = hacker2:getEnergy()
    h3Energy = hacker3:getEnergy()
    h4Energy = hacker4:getEnergy()
    h5Energy = hacker5:getEnergy()
    h6Energy = hacker6:getEnergy()
    h7Energy = hacker7:getEnergy()
    hacker8 = hacker8:getEnergy()
    h9Energy = hacker9:getEnergy()
end
...
Warspite:addInfos(3,"Number of Personnel", "184")
Warspite:addInfos(4, "Life signs", "Strong")
Warspite:setScanned(true)
Scarab = CpuShip():setFaction("UCN"):setTemplate("UCS Skirmish Frigate"):setCallSign("UCS Scarab"):setPosition(105582,46109):setHeading(280):setCanBeDestroyed(true):orderIdle():setWeaponStorage("Homing", 15):setDescriptions("Skirmish Class Frigate", "Current orders: Return to homebase."):setCanBeDestroyed(false)
...
minifight1 = CpuShip():setFaction("Independent"):setTemplate("Interceptor"):setCallSign("Dialup noises"):setPosition(105216, 40144):setRadarSignatureInfo(0.5, 0.9, 0.0):setEnergy(10)
    ...
    minifight1:setDescriptionForScanState("fullscan", "Justin time to get shot by us")
...
minifight2 = CpuShip():setFaction("Independent"):setTemplate("Interceptor"):setCallSign("Beep Boop"):setPosition(113040, 45369):setRadarSignatureInfo(0.5, 0.9, 0.0)
    ...
    minifight2:addInfos(2, "Energy readout", tostring(mf2Energy))
minifight3 = CpuShip():setFaction("Independent"):setTemplate("Interceptor"):setCallSign("Whirr whirr"):setPosition(100000, 50000):setRadarSignatureInfo(0.5, 0.9, 0.0)
    ...
    minifight3:setDescriptionForScanState("simplescan", "Go screw yourself")
    minifight3:setDescriptionForScanState("fullscan", "This isn't a joke. Go screw yourself.")
    minifight3:addInfos(1, "Life signs", "Nil")
    minifight3:addInfos(2, "Energy readout", tostring(mf3Energy))
...
--enemy ships hiding in nebulas
    ...
    hacker1:addInfos(2, "Energy readout", string.gsub("Energyread", "Energyread", tostring(h1Energy)))
    ...
    hacker2:addInfos(2, "Energy readout", string.gsub("Energyread", "Energyread", tostring(h2Energy)))
    ...
    hacker3:addInfos(2, "Energy readout", string.gsub("Energyread", "Energyread", tostring(h3Energy)))
    ...
    hacker4:addInfos(2, "Energy readout", string.gsub("Energyread", "Energyread", tostring(h4Energy)))
    ...
    hacker5:addInfos(2, "Energy readout", string.gsub("Energyread", "Energyread", tostring(h5Energy)))
    ...
    hacker6:addInfos(2, "Energy readout", string.gsub("Energyread", "Energyread", tostring(h6Energy)))
    ...
    hacker7:addInfos(2, "Energy readout", string.gsub("Energyread", "Energyread", tostring(h7Energy)))
    ...
    hacker8:addInfos(2, "Energy readout", string.gsub("Energyread", "Energyread", tostring(h8Energy)))
    ...
    hacker9:addInfos(2, "Energy readout", string.gsub("Energyread", "Energyread", tostring(h9Energy)))
...
siphonscanstate = Siphon:isScannedBy(player)
    if siphonscanstate == true then
           zone = Zone():setPoints(228692, 204750, 236296, 206211, 242287, 210792, 246661, 216680, 248225, 224596, 247026, 231316, 242964, 237929, 237233, 241994, 228640, 244388, 220826, 242619, 214421, 238242, 211190, 231681, 208795, 224595, 210200, 216940, 214473, 210428, 221191, 206211)
           :setColor(0, 0, 0)
    end
end
...
function asteroidInfo()
    ...
    -- this gets streoids 50 grids (1mil pixels) within starting point (0.0). Change values as needed
    ...
    for _, obj in ipairs(asteroidList) do
        obj:setSize(math.random(50, 500)):setRadarSignatureInfo(0.8, 0.0, 0.0)
...
function triggerGMDockWithPlayer()
    ...
    addGMFunction(GMDisappearDockednew, triggerGMDisappearDockednew)
end
