...
: GuiElement(owner, "COMMS_OVERLAY")
{
    ...
    chat_comms_send_button = new GuiButton(chat_comms_box, "SEND_BUTTON", tr("comms","Send"), [this]() {
        if (my_spaceship)
            my_spaceship->commandSendCommPlayer(chat_comms_message_entry->getText());
        chat_comms_message_entry->setText("");
    });
    ...
    chat_comms_close_button = new GuiButton(chat_comms_box, "CLOSE_BUTTON", tr("comms","Close"), [this]() {
        if (my_spaceship)
            my_spaceship->commandCloseTextComm();
    });
